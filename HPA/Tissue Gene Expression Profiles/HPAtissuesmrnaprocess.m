...
gene_atb.cm = cminit(20344, 32, [], 'GeneSym', [], 'Ensemble Acc', [], [], [], 'Tissue', [], [], [], [], []);

% read file
fid = fopen('input/rnaseq_32tissues.txt', 'r');

currline = fgetl(fid);

currcells = strsplit(currline, '\t', 'CollapseDelimiters', false);

gene_atb.cm.entry = currcells(3:gene_atb.cm.numentries+2)';

for i = 1:1:gene_atb.cm.numentries
    gene_atb.cm.entry{i} = strrep(gene_atb.cm.entry{i}, ' fpkm', '');
end
...
for i = 1:1:gene_atb.cm.numterms
    ...
    gene_atb.cm.matrix(i,:) = str2double(currcells(3:gene_atb.cm.numentries+2));
    
end

fclose(fid);

save('input/gene_tissue_hpa_mRNA_unmodified.mat', '-struct', 'gene_atb');
%}
% 20344 x 32
gene_atb = load('input/gene_tissue_hpa_mRNA_unmodified.mat', '-mat', 'cm');



% map gene symbols to entrez gene symbols and discard rows corresponding to
% un-mapped symbols (1035 rows discarded, 19309 x 32 remaining)
...
if sum(isnan(gene_atb.cm.matrix(:))) > 0 % FALSE
    ...
    % rows discarded, 0 columns discarded. 19309 x 32 remaining)
...
if sum(isnan(gene_atb.cm.matrix(:))) > 0 % TRUE
    % remove rows and columns with more than 75% undetected values (1069
    % rows discarded, 0 columns discarded. 18240 x 32 remaining)
...
if numel(unique(gene_atb.cm.term)) < gene_atb.cm.numterms % TRUE
    % merge measurements corresponding to the same gene (18140 unique gene
    % symbols out of 18240 total gene symbols, 18140 x 32 remaining)
...
% absolute deviation (mad) (discarded 644 rows and 1 col, 17496 x 31 remaining)
...
% daviesbouldin = cm2daviesbouldin(cmtranspose(gene_atb.cm)); % no column group labels, can't compute dbi
