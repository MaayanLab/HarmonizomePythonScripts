...
atb_gene.lists = listsinit(301, [], 'Pathway', [], 'KEGG ID', [], [], [], 'GeneID', [], [], [], 'GeneID', false, []);

% read data
fid = fopen('input/dataset_20160406_original.gmt', 'r');
for i = 1:1:atb_gene.lists.numterms
    currline = fgetl(fid);
    currcells = strsplitbyadr(currline, '\t');
    atb_gene.lists.term{i} = currcells{2};
    atb_gene.lists.termdesc{i} = currcells{3};
    atb_gene.lists.entries{i} = currcells(6:end)';
    atb_gene.lists.entryids{i} = str2double(currcells(6:end)');
