...
gene_atb.edges = edgesinit(91458, [], 'GeneSym', [], 'Ensemble Acc', [], [], [], 'Cellular Component', [], 'GO', [], [], true, []);



% read file
fid = fopen('input/human_compartment_experiments_full_20150202.tsv', 'r');
...
for i = 1:1:gene_atb.edges.numedges
    ...
    gene_atb.edges.weight(i) = str2double(currcells{7});  % confidence score (number of "stars" rating)
    
end

fclose(fid);

% discard duplicate edges (retain the edge with the highest score), 91458
% edges remaining (there were no duplicates)
gene_atb.edges = edgesunique(gene_atb.edges);

save('input/gene_compartment_jensenexpts_unprocessed_20150202.mat', '-struct', 'gene_atb');
%}
gene_atb = load('input/gene_compartment_jensenexpts_unprocessed_20150202.mat', '-mat', 'edges');
...
% map compartments to go cellular component ontology and discard un-mapped compartments
...
if numel(unique(gene_atb.cm.entry)) < gene_atb.cm.numentries
    % merge measurements corresponding to the same atb
