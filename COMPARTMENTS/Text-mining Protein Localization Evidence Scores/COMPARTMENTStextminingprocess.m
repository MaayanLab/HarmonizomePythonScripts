...
gene_atb.edges = edgesinit(402133, [], 'GeneSym', [], 'Ensemble Acc', [], [], [], 'Cellular Component', [], 'GO', [], [], true, []);



% read file
fid = fopen('input/human_compartment_textmining_full_20150202.tsv', 'r');
...
for i = 1:1:gene_atb.edges.numedges
    ...
    gene_atb.edges.weight(i) = str2double(currcells{5});  % zscore
    
end

fclose(fid);

% discard duplicate edges (retain the edge with the highest score), 402133
% edges remaining (no duplicates)
gene_atb.edges = edgesunique(gene_atb.edges);

save('input/gene_compartment_jensentextmining_unprocessed_20150202.mat', '-struct', 'gene_atb');
%}
gene_atb = load('input/gene_compartment_jensentextmining_unprocessed_20150202.mat', '-mat', 'edges');
