...
gene_atb.edges = edgesinit(679121, [], 'HGNC ID', [], 'UniprotAcc', [], 'GeneID', [], 'Cellular Component', [], 'GO ID', [], [], true, []);


% read data
fid = fopen('input/parse_results_predictions_fixed.txt', 'r');
...
for i = 1:1:gene_atb.edges.numedges
    ...
    gene_atb.edges.weight(i) = str2double(currcells{8});
    
end

fclose(fid);

discard = gene_atb.edges.weight == 0 | (strcmp(gene_atb.edges.source, '-666') & strcmp(gene_atb.edges.sourcedesc, '-666') & gene_atb.edges.sourceid == -666) | strcmp(gene_atb.edges.targetdesc, '-666');
gene_atb.edges = edgesdiscard(gene_atb.edges, discard);
save('input/gene_cc_locate_predicted_unprocessed_20150730.mat', '-struct', 'gene_atb');
%}

% load data
% gene_atb = load('input/gene_cc_locate_predicted_unprocessed_20150730.mat', '-mat', 'edges');
